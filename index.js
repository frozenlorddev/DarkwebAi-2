...
const { useMultiFileAuthState, DisconnectReason } = require('@ostyado/baileys');
...
// Root: dangerous theme (same as before – kept short for brevity; full version in previous message)
app.get('/', (req, res) => {
...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    body { background:#000; font-family:'JetBrains Mono',monospace; color:#ff3333; min-height:100vh; padding:20px; }
    .container { max-width:1000px; margin:0 auto; background:rgba(5,5,5,0.95); border:1px solid #ff0000; border-radius:20px; padding:2rem; box-shadow:0 0 30px rgba(255,0,0,0.3); }
...
<div class="container">
  ...
  <footer>DARKWEB AI – TOTAL ANNIHILATION MODE // DEVELOPERS: ${OWNERS.join(', ')}</footer>
</div>
<script>
  ...
  async function loadStatus() {
    try { const d = await fetchJSON('/api/status'); document.getElementById('statSessions').innerText = d.sessions || 0; document.getElementById('statUptime').innerText = formatTime(d.uptime || 0); } catch(e) { console.warn(e); }
  }
  function formatTime(sec) { const h = Math.floor(sec/3600), m = Math.floor((sec%3600)/60); return h>0 ? h+'h '+m+'m' : m+'m'; }
  async function loadSessions() {
    try {
      ...
      let html = '';
      for(let s of list) html += \`<div class="session-item"><span><strong>\${s.phoneNumber}</strong> \${s.status === 'connected' ? '✅' : '⟳'}</span><button class="delete-btn" onclick="deleteSession('\${s.phoneNumber}')">DELETE</button></div>\`;
  ...
  document.getElementById('pairBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const data = await res.json();
      if(data.code) { document.getElementById('pairCode').innerText = data.code; document.getElementById('codeBox').style.display = 'block'; } else alert('ERROR: ' + (data.error || 'UNKNOWN'));
      loadSessions(); loadStatus();
    } catch(e) { alert('SERVER ERROR: ' + e.message); }
    btn.disabled = false; btn.innerText = '⚡ EXECUTE PAIRING ⚡';
  });
  document.getElementById('copyBtn').addEventListener('click', () => { const code = document.getElementById('pairCode').innerText; navigator.clipboard.writeText(code).then(() => alert('💀 CODE COPIED 💀')); });
...
async function startWhatsAppSessionAndGetCode(sessionId, phoneNumber) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            setTimeout(() => { if (!resolved) reject(new Error('Connection timeout')); }, 30000);
        } catch (err) {
            reject(err);
        }
    });
...
server.listen(PORT, () => {
    ...
    console.log(`🤖 WhatsApp pairing ready (no delay)`);
    console.log(`◣━━━━━━━━━━━━━━━━━━━━━━◢\n`);
});
